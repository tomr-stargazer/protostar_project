...
sed = readSed.readSed('mondo_sed_new.dat', sep='|')

### Parameters of physical model

lstar    = 7                 # Stellar luminosity in units of solar luminosities
...
abund[(tempx > 90.0).nonzero()] = 1.0e-10
abund[(tempx < 90.0).nonzero()] = 1.0e-11

pdb.set_trace()

transphereRatran.ratranRun(r=rx, rho=rhox, temp=tempx, db=3, abund=abund, dpc=dist,
                           trans='3', pixel=0.1, molfile='h13cn.dat', writeonly=0, skyonly=0, 
