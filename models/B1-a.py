...
from transphere_python import readSed

sed = readSed.readSed('B1a_mondo_sed.dat', sep='|')

### Parameters of physical model

lstar    = 1.3               # Stellar luminosity in units of solar luminosities
...
menv     = 2.8               # Envelope mass in M_sun
plrho    = -0.7              # Powerlaw for rho
...
nr       = 100               # Nr of radial shells for grid
nref     = 50               # Nr of refinement points
...
plrho_list = [-1.9, -1.5, -1.2, -1, -0.7]

for plrho in plrho_list:
    ...
    plt.figure(1)
    plotTransphere.plotSpectrum(s, dpc=dist, jy=1, pstyle='-')
    plt.xscale('log')
    plt.yscale('log')
    plt.xlim((1, 3.0e3))
    ...
    plt.figure(3)
    plotTransphere.plotTemperature(a, -1, pstyle='-')
...
from transphere_python import transphereRatran
